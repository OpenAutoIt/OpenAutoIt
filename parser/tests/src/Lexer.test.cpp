...
// TODO: Lexer tests are completly broken
...
{
    const OpenAutoIt::Lexer lexer{"Hello World"};
...
TEST_CASE("Lexer - only '$' is not a valid VariableIdentifier")
{
    const phi::string_view source{"$"};

    res = lexer.ProcessString(source);

    TOKEN_MATCHES(res.at(0u), Garbage, "$", 1u, 1u);
}
